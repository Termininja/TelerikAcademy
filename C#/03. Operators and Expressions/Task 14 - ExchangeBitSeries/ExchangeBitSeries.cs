//Task 14*: Write a program that exchanges bits {p, p+1, …, p+k-1) with
...
class ExchangeBitSeries
{
    static void Main()
    {
        /* Short variant */
        //uint rank = 1;
        //for (int s = 1; s <= k; s++) rank = 2 * rank;
        //long result = (
        //    (number & ((rank - 1) << q)) >> (q - p)) |
        //    ((number & ((rank - 1) << p)) << (q - p)) | 
        //    (number & (~(((rank - 1) << p) | ((rank - 1) << q))));
        ...
        while (true)
        {
            ...
            k:
                ...
                Console.ResetColor();
                if (str == "")                                      // if the "Enter" is pressed
                {
                    Console.ResetColor();
                    goto k;
                }
                int k = int.Parse(str);                             // take the "k" value from the 'str' string
                if (k > length / 2)                                 //checks if the 'k' is too big for the number 'number'
                {
            ...
            from_position:
                ...
                if (p > (length / 2) - k)                           // checks if the position 'p' is too big for the 'number'
                {
                    Console.ForegroundColor = ConsoleColor.Red;
            ...
            to_position:
                ...
                if (q > length - k | q < p + k - 1)                 // checks if the 'q' is too big for the number 'number'
                {
                    Console.ForegroundColor = ConsoleColor.Red;
                ...
                uint mask1 = (rank - 1) << p;                       // use mask1 to backup the 1st group of bits (3, 4 and 5) 
                uint bits_group1 = (number & mask1) >> p;           // the 1st 3 bits (3, 4 and 5) from group1

                uint mask2 = (rank - 1) << q;                       // use mask2 to backup the 2nd group of bits (24, 25 and 26) 
                uint bits_group2 = (number & mask2) >> q;           // the 2nd 3 bits (24, 25 and 26) from group2
