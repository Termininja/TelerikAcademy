...
class FallingRocksGame
{
    ...
    // Sets the width and the height of console
    ...
    static void CreateRocks()
    {
        ...
        // The rock object will enter in the RocksList
        if (!(NewRock.x <= wall_width1 + 1 | NewRock.x >= Console.WindowWidth - wall_width2 - 1)) RocksList.Add(NewRock);
    }
    ...
    static void CreateBonusLife()
    {
        ...
        // Adds this object in the BonusLifeList
        if (!(BonusLife.x <= wall_width1 + 1 | BonusLife.x >= Console.WindowWidth - wall_width2 - 1)) BonusLifeList.Add(BonusLife);
    }
    ...
    static void CreateBonusClear()
    {
        ...
        // Adds this object in the BonusClearList
        if (!(BonusClear.x <= wall_width1 + 1 | BonusClear.x >= Console.WindowWidth - wall_width2 - 1)) BonusClearList.Add(BonusClear);
    }

    // Sets a different difficulty for different level
    static void LevelDifficulty()
    {
        if (t % 2 == 0) diff_temp1++;                               // creates one sub-timer
        if (t % 3 == 0) t_level++;                                  // creates a second sub-timer
        if (t_level == 100)                                         // we use the 2nd sub-timer to measure the time in each level
        {
            ...
            if (level <= 10 | speed < 100) speed += 10;             // increase the speed by 10 (max 100)
        }

        // Schedule for big rocks
        if ((t <= 600 & t >= 60) | (level < 8 & level >= 5) | level >= 12 & level <= 30)    // big rocks takes part only in these levels
        {
            if ((level >= 2 & level < 5) | level == 7 | level >= 13) diff = diff_temp1;     // more difficulty for these levels
            else if (level < 2 | (level >= 5 & level < 7) | level > 11 & level <= 13) diff = t_level;
            if (t_level % 20 == 0 & diff_temp2 != diff) CreateBigRocks();
        }
        ...
        if (level >= 3)
        {
            ...
            else if (level > 11)                                    // after lv-12 we need a lot of bonuses because of speed
            {
                ...
                {
                    CreateBonusLife();
                    if (level <= 20) CreateBonusClear();            // no more clear bonuses after lv-20
                }
            }
            ...
            if (level < 12 | level >= 24)
            {
                if (level <= 10 | level >= 24)
                {
                    ...
                    {
                        // From lv-5 to lv-7 the gap between the walls is ~ 50
                        if (Console.WindowWidth - (wall_width1 + wall_width2) <= 50)
                        {
                            wall_width1--;
                            wall_width2--;
                        }
                        if (wall_width1 <= 0) wall_width1 = 0;
                        if (wall_width2 <= 0) wall_width2 = 0;
                    }
                    else
                    {
                        if (t >= 600)                               // this increases the width of the left wall when the base timer is 600
                        {
                            if (wall_width1 < 30) wall_width1++;
                        }
                        if (t >= 500)
                        {
                            if (wall_width2 < 40) wall_width2++;
                        }
                        else
                        {
                            if (wall_width1 < 10) wall_width1++;
                            if (wall_width2 < 10) wall_width2++;
                        }
                        if (level < 30)
                        {
                            // The gap between the walls will be ~ 6
                            if (Console.WindowWidth - (wall_width1 + wall_width2) <= 6)
                            {
                                wall_width1--;
                                wall_width2--;
                            }
                        ...
                        {
                            // The gap between the walls will be ~ 20
                            if (Console.WindowWidth - (wall_width1 + wall_width2) <= 20)
                            {
                                wall_width1--;
                                wall_width2--;
                            }
                ...
                else
                {
                    wall_width1--;
                    wall_width2--;
                    if (wall_width1 <= 0) wall_width1 = 0;          // the width of the left wall can't be negative
                    if (wall_width2 <= 0) wall_width2 = 0;          // the width of the right wall can't be negative
                }
                CreateLeftSideWall();
                CreateRightSideWall();
            }
        ...
        // The rocks are displayed in the whole time
        CreateRocks();
    }
    ...
    {
        foreach (Object rock in RocksList) PrintObject(rock.x, rock.y, rock.str, rock.col);
        foreach (Object bigrock in BigRocksList) PrintObject(bigrock.x, bigrock.y, bigrock.str, bigrock.col);
        foreach (Object bonus_life in BonusLifeList) PrintObject(bonus_life.x, bonus_life.y, bonus_life.str, bonus_life.col);
        foreach (Object bonus_clear in BonusClearList) PrintObject(bonus_clear.x, bonus_clear.y, bonus_clear.str, bonus_clear.col);
        foreach (Object bricks1 in WallList1) PrintObject(bricks1.x, bricks1.y, bricks1.str, bricks1.col);
        foreach (Object bricks2 in WallList2) PrintObject(bricks2.x, bricks2.y, bricks2.str, bricks2.col);
    }
    ...
    static void Main()
    {
        ...
        while (true)
        {
            ...
            if (Console.KeyAvailable)
            {
                ...
                // Moves dwarf to left
                if (key.Key == ConsoleKey.LeftArrow && Dwarf.x > 0) Dwarf.x--;

                // Moves dwarf to right
                else if (key.Key == ConsoleKey.RightArrow && Dwarf.x < Console.WindowWidth - 2) Dwarf.x++;

                // Pause the game with <Spacebar>
                else if (key.Key == ConsoleKey.Spacebar) Console.ReadKey();
            }

            // Move and clear the rock objects
            for (int i = 0; i < RocksList.Count; i++)
            {
                ...
                if (new_rock.y < Console.WindowHeight) RocksList.Insert(i, new_rock);
                else i = -1;
            ...
            // Move and clear the bonus "life" objects
            for (int bl = 0; bl < BonusLifeList.Count; bl++)
            {
                ...
                if (new_bonus_life.y < Console.WindowHeight) BonusLifeList.Insert(bl, new_bonus_life);
                else bl = -1;
                BonusLifeList.Remove(old_bonus_life);
                if (
                    old_bonus_life.y + 1 == Dwarf.y &&
                    old_bonus_life.x >= Dwarf.x & old_bonus_life.x <= Dwarf.x + Dwarf.str.Length - 1 &&
                    lives < 5)
                    lives++;
            }

            // Move and clear the bonus "clear" objects
            for (int bc = 0; bc < BonusClearList.Count; bc++)
            {
                ...
                if (new_bonus_clear.y < Console.WindowHeight) BonusClearList.Insert(bc, new_bonus_clear);
                else bc = -1;
            ...
            // Move and clear the big rock objects
            if (diff_temp2 != t_level)
            {
                ...
                for (int m = 0; m < BigRocksList.Count; m++)
                {
                    ...
                    if (new_bigrock.y < Console.WindowHeight) BigRocksList.Insert(m, new_bigrock);
                    else m = -1;
                    BigRocksList.Remove(old_bigrock);
                    if (
                        old_bigrock.y + 1 == Dwarf.y &&
                        (
                        (Dwarf.x >= old_bigrock.x && Dwarf.x < old_bigrock.x + old_bigrock.w) ||
                        (Dwarf.x + Dwarf.str.Length > old_bigrock.x && Dwarf.x + Dwarf.str.Length < old_bigrock.x + old_bigrock.w)
                        )
                       )
                    {
                        hitted = true;
            ...
            // Move and clear the objects from the left wall
            for (int j = 0; j < WallList1.Count; j++)
            {
                ...
                if (new_brick1.y < Console.WindowHeight) WallList1.Insert(j, new_brick1);
                else j = -1;
            ...
            for (int k = 0; k < WallList2.Count; k++)
            {
                ...
                if (new_brick2.y < Console.WindowHeight) WallList2.Insert(k, new_brick2);
                else k = -1;
        ...
        hitted:
            ...
            else PrintObject(Dwarf.x, Dwarf.y, Dwarf.str, Dwarf.col);   // the dwarf is printed only if it is not hitted
