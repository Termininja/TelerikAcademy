// Task 18. * Write a program that reads an array of integers and removes from it a minimal number of elements
...
class IncreasingOrder
{
    static void Main()
    {
        ...
        for (int i = 0; i < Math.Pow(2, array.Length); i++)
        {
            ...
            for (int b = 0; b < array.Length; b++)          // for each bit in the number 'i'
            {
                ...
                if (str[b] == '1') temp.Add(array[b]);      // if the bit is equqal to 1
            }

            // If the current temp array is sorted
            if (IsSorted(temp))
            {
                // If the length is larger than previous temp array
    ...
    static bool IsSorted(List<int> temp)
    {
        ...
        {
            if (temp[j] >= temp[j - 1]) sorted = true;
            else
            {
                sorted = false;
                break;
